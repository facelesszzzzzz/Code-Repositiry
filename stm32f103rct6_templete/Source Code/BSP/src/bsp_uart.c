...
{  
	static uint16_t i,j;
	Uart_Data_t *sUart3 = GetUart_Data_t(Uart3_NUM);
	va_list ap; 
	va_start(ap,fmt);
	vsprintf((char*)sUart3->TxBuf,fmt,ap);
	va_end(ap);
	i=strlen((const char*)sUart3->TxBuf);		//此次发送数据的长度
	for(j=0;j<i;j++)							//循环发送数据
	{
	  	while(USART_GetFlagStatus(USART3,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕   
		USART_SendData(USART3,sUart3->TxBuf[j]);
	} 
}
#endif
